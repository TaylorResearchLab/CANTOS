cluster_label_assignment <- function(affinity_cluster_nested){
  ...
  affinity_cluster_nested$suggested_cluster_label <- NA
  ...
  for (iter in 1:length(subcluster_id_list)){
    ...
    affinity_cluster_nested$suggested_cluster_label[index_affinity]<- paste(assigned_cluster_labels,collapse = ";")
  }
