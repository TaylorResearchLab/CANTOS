...
disease_transform<- read.csv(paste(intermediate_dir,"/disease_transform_pca_v3_5thed.csv",sep="") )
...
save.image(paste(analysis_dir,"/LTE3_AP/6A-LTE3_5th.RData",sep=""))
...
for (iter in 1: length(af_clust_euclidean@clusters)){
  affinity_cluster_v3_df[iter,1] <- paste(names(unlist(af_clust_euclidean@clusters[iter])),collapse = "@")
...
#affinity_cluster_v3_df$Cluster_Total_Members <- NA
cluster_frequency_table <- as.data.frame(table(affinity_cluster_v3_df$Cluster_ID))
...
while(length(large_cluster_labels)>0){
  ...
  for(iter in 1:length(large_cluster_labels)){
    ...
    subset_embedding_df <- as.data.frame(affinity_cluster_v3_df$Tumor_Names[affinity_cluster_v3_df$Cluster_ID==Clusters_Names])
    ...
    if(flag_converge=="No"){
      for (iter_nested_affinity_cluser in 1: dim(subset_affinity_df)[1]){
        ind_location <- which (affinity_cluster_v3_df$Tumor_Names==subset_affinity_df$Tumor_Names[iter_nested_affinity_cluser])
        affinity_cluster_v3_df$Cluster_ID[ind_location]<-subset_affinity_df$SubCluster_ID[iter_nested_affinity_cluser]
      }
