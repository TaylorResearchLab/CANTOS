...
save.image(paste(run_time_analysis,"/cosine_AP/cosine_AP_5th.RData",sep=""))
...
who_5th_match_cosine_df <- cbind(rownames(dissimilarity_matrix_cosine_who_5th))
colnames(who_5th_match_cosine_df)<-"Tumor_Names"
who_5th_match_cosine_df <-as.data.frame(who_5th_match_cosine_df)
who_5th_match_cosine_df$WHO_Matches<- NA
who_5th_match_cosine_df$WHO_distance<-NA

for (iter in 1: dim(who_5th_match_cosine_df)[1]){
...
nested_affinity_cluster_cosine<- nested_affinity_cluster_cosine %>% dplyr::left_join(who_5th_match_cosine_df,by="Tumor_Names")
...
Runtime_cosine_AP_5th = round(elapsed_time, 2)
Memory_gb_cosine_AP_5th=round(sum(sizes) / (1024^2), 3)/1000
