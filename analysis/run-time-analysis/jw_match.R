...
# Jaro-Winkler distance between tumors 
...
dissimilarity_matrix_jw<-foreach(iter=1:length(df_tumor_names),.combine=rbind) %dopar% {
  #dissimilarity_matrix_jw<-foreach(iter=1:20,.combine=rbind) %dopar% {
  print(iter)
...
#rownames(dissimilarity_matrix_jw) <- df_tumor_names[1:20]
rownames(dissimilarity_matrix_jw) <- df_tumor_names
colnames(dissimilarity_matrix_jw) <- df_tumor_names
stopCluster(cl)


df_tumor_names<- colnames(dissimilarity_matrix_jw)
normalizing_matrix_jw <- as.data.frame(matrix(nrow=length(df_tumor_names),ncol=length(df_tumor_names)))
#normalizing_matrix_jw <- as.data.frame(matrix(nrow=20,ncol=length(df_tumor_names)))
#rownames(normalizing_matrix_jw)<-df_tumor_names[1:20]
rownames(normalizing_matrix_jw)<-df_tumor_names
colnames(normalizing_matrix_jw)<-df_tumor_names

cl <- makeCluster(5, outfile="")
registerDoParallel(cl)
normalizing_matrix_jw<-foreach(iter=1:length(df_tumor_names),.combine=rbind) %dopar% {
  #normalizing_matrix_jw<-foreach(iter=1:20,.combine=rbind) %dopar% {
  print(iter)
  disease_name <- colnames(dissimilarity_matrix_jw)[iter]
...
rownames(normalizing_matrix_jw) <- df_tumor_names
colnames(normalizing_matrix_jw) <- df_tumor_names
dissimilarity_matrix_jw<-dissimilarity_matrix_jw/normalizing_matrix_jw


dissimilarity_matrix_jw<-as.data.frame(dissimilarity_matrix_jw)

dissimilarity_matrix_jw_who_all <- dissimilarity_matrix_jw %>% dplyr::select(one_of(WHO_Terms_All$Tumor_Names))
...
index_min_who_all_jw <- as.matrix(apply(dissimilarity_matrix_jw_who_all, 1, which.min))
who_all_match_jw_df <- as.data.frame(cbind(rownames(dissimilarity_matrix_jw_who_all)))

index_min_who_5th_jw <- as.matrix(apply(dissimilarity_matrix_jw_who_5th, 1, which.min))
who_5th_match_jw_df <- as.data.frame(cbind(rownames(dissimilarity_matrix_jw_who_5th)))

index_min_ncit_jw <- as.matrix(apply(dissimilarity_matrix_jw_ncit, 1, which.min))
ncit_match_jw_df <- as.data.frame(cbind(rownames(dissimilarity_matrix_jw_ncit)))


who_all_match_jw_df$WHO_Matches<- NA
who_all_match_jw_df$WHO_distance<-NA

who_5th_match_jw_df$WHO_Matches<- NA
who_5th_match_jw_df$WHO_distance<-NA

ncit_match_jw_df$WHO_Matches<- NA
ncit_match_jw_df$WHO_distance<-NA

for (iter in 1: dim(who_all_match_jw_df)[1]){
  
  who_all_match_jw_df$WHO_Matches[iter] <- colnames(dissimilarity_matrix_jw_who_all)[index_min_who_all_jw[iter]]
  who_all_match_jw_df$WHO_distance[iter]<-dissimilarity_matrix_jw_who_all[iter,index_min_who_all_jw[iter]]
