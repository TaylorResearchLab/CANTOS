...
# cosine distance between tumors 
...
dissimilarity_matrix_cosine<-foreach(iter=1:length(df_tumor_names),.combine=rbind) %dopar% {
  #dissimilarity_matrix_cosine<-foreach(iter=1:20,.combine=rbind) %dopar% {
  print(iter)
...
#rownames(dissimilarity_matrix_cosine) <- df_tumor_names[1:20]
rownames(dissimilarity_matrix_cosine) <- df_tumor_names
colnames(dissimilarity_matrix_cosine) <- df_tumor_names
stopCluster(cl)


df_tumor_names<- colnames(dissimilarity_matrix_cosine)
normalizing_matrix_cosine <- as.data.frame(matrix(nrow=length(df_tumor_names),ncol=length(df_tumor_names)))
#normalizing_matrix_cosine <- as.data.frame(matrix(nrow=20,ncol=length(df_tumor_names)))
#rownames(normalizing_matrix_cosine)<-df_tumor_names[1:20]
rownames(normalizing_matrix_cosine)<-df_tumor_names
colnames(normalizing_matrix_cosine)<-df_tumor_names

cl <- makeCluster(5, outfile="")
registerDoParallel(cl)
normalizing_matrix_cosine<-foreach(iter=1:length(df_tumor_names),.combine=rbind) %dopar% {
  #normalizing_matrix_cosine<-foreach(iter=1:20,.combine=rbind) %dopar% {
  print(iter)
  disease_name <- colnames(dissimilarity_matrix_cosine)[iter]
...
rownames(normalizing_matrix_cosine) <- df_tumor_names
colnames(normalizing_matrix_cosine) <- df_tumor_names
dissimilarity_matrix_cosine<-dissimilarity_matrix_cosine/normalizing_matrix_cosine
...
dissimilarity_matrix_cosine_who_5th <- dissimilarity_matrix_cosine %>% dplyr::select(one_of(WHO_Terms_5th$Tumor_Names))
...
who_all_match_cosine_df <- as.data.frame(cbind(rownames(dissimilarity_matrix_cosine_who_all)))

index_min_who_5th_cosine <- as.matrix(apply(dissimilarity_matrix_cosine_who_5th, 1, which.min))
who_5th_match_cosine_df <- as.data.frame(cbind(rownames(dissimilarity_matrix_cosine_who_5th)))

index_min_ncit_cosine <- as.matrix(apply(dissimilarity_matrix_cosine_ncit, 1, which.min))
ncit_match_cosine_df <- as.data.frame(cbind(rownames(dissimilarity_matrix_cosine_ncit)))


who_all_match_cosine_df$WHO_Matches<- NA
who_all_match_cosine_df$WHO_distance<-NA

who_5th_match_cosine_df$WHO_Matches<- NA
who_5th_match_cosine_df$WHO_distance<-NA

ncit_match_cosine_df$WHO_Matches<- NA
ncit_match_cosine_df$WHO_distance<-NA
...
for (iter in 1: dim(who_all_match_cosine_df)[1]){
  ...
  who_5th_match_cosine_df$WHO_Matches[iter] <- colnames(dissimilarity_matrix_cosine_who_5th)[index_min_who_5th_cosine[iter]]
  who_5th_match_cosine_df$WHO_distance[iter]<-dissimilarity_matrix_cosine_who_5th[iter,index_min_who_5th_cosine[iter]]
