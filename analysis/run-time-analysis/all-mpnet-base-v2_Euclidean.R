method_name <- "all-mpnet-base-v2+Euclidean Distance"
start_time <- Sys.time()
...
print("mpnet Distance Matrix Computed")

save.image("mpnet.RData")

start_time<-Sys.time()

distance_matrix_all_mpnet_base_v2<-as.data.frame(distance_matrix_all_mpnet_base_v2) # EXECUTED

distance_matrix_all_mpnet_base_v2_all <- distance_matrix_all_mpnet_base_v2 %>% dplyr::select(all_of(WHO_Terms_All$Tumor_Names))
distance_matrix_all_mpnet_base_v2_5th <- distance_matrix_all_mpnet_base_v2 %>% dplyr::select(all_of(WHO_Terms_5th$Tumor_Names))
distance_matrix_all_mpnet_base_v2_ncit <- distance_matrix_all_mpnet_base_v2 %>% dplyr::select(all_of(ncit_terms$Tumor_Names))

print("all mpnet match")
nomic_match_all<- nearest_match_embeddings(distance_matrix_all_mpnet_base_v2_all,"mpnet") # Executed
print("5th mpnet match ")
nomic_match_5th<- nearest_match_embeddings(distance_matrix_all_mpnet_base_v2_5th,"mpnet") # Executed
print("ncit mpnet match ")
nomic_match_ncit<- nearest_match_embeddings(distance_matrix_all_mpnet_base_v2_ncit,"mpnet") # Executed
end_time <- Sys.time()
elapsed_time4<- as.numeric(difftime(end_time, start_time, units = "secs"))

print("mpnet Computed")
