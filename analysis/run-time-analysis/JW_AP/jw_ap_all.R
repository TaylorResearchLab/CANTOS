...
apclust_jw_all <- apcluster(simmilarity_matrix_jw) #11:19 am -2:44 pm 
affinity_cluster_jw_all_df<-as.data.frame(matrix(nrow=1,ncol=2))
colnames(affinity_cluster_jw_all_df)<-c("Tumor_Names","Cluster_ID")

for (iter in 1: length(apclust_jw_all@clusters)){
  affinity_cluster_jw_all_df[iter,1] <- paste(names(unlist(apclust_jw_all@clusters[iter])),collapse = "@")
  affinity_cluster_jw_all_df[iter,2] <- iter
}
affinity_cluster_jw_all_df<- affinity_cluster_jw_all_df %>% separate_rows(Tumor_Names, sep = '@')

end_time<-Sys.time()
elapsed_time_1 <- as.numeric(difftime(end_time, start_time, units = "secs"))
save.image(paste(run_time_analysis,"/JW_AP/jw_AP_ALL.RData",sep=""))

start_time2 <- Sys.time()


nested_affinity_cluster_jw<- edit_distance_nested_cluster(affinity_cluster_jw_all_df,simmilarity_matrix_jw)
...
who_all_match_jw_df <- cbind(rownames(dissimilarity_matrix_jw_who_all))
colnames(who_all_match_jw_df)<-"Tumor_Names"
who_all_match_jw_df <-as.data.frame(who_all_match_jw_df)
...
nested_affinity_cluster_jw<- nested_affinity_cluster_jw %>% dplyr::left_join(who_all_match_jw_df,by="Tumor_Names")
...
Runtime_jw_AP_all = round(elapsed_time, 2)
Memory_gb_jw_AP_all=round(sum(sizes) / (1024^2), 3)/1000
