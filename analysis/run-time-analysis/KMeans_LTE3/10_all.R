...
who_ncit_match_v3<-read.csv(paste(analysis_dir,"/results/who_ncit_match_v3_all.csv",sep=""))
who_ncit_match_v3<-who_ncit_match_v3[,c(-1)]
# Join the matches to Kmeans 
colnames(kmeans_clust_result_embedding_V3)[3]<-"Cluster_ID"
kmeans_clust_result_embedding_V3<-kmeans_clust_result_embedding_V3 %>% dplyr::select(nct_id,Tumor_Names,Cluster_ID)
...
save.image(paste(run_time_analysis,"/KMeans_LTE3/10_all.RData",sep=""))
