method_name <- "embed-english-v2.0+Euclidean Distance"
start_time <- Sys.time()
...
print("cohere Loading")
cohere_embeddings<-read.csv(paste(data_dir,"/Embeddings/cohere_embeddings_embed_english_v2.csv",sep="")) 
end_time <- Sys.time()
elapsed_time<- as.numeric(difftime(end_time, start_time, units = "secs"))

start_time<-Sys.time()
cohere_embeddings<-cohere_embeddings %>% group_by(Tumor_Names) %>% summarise_all("mean")
end_time <- Sys.time()
...
print("cohere Distance Matrix")
distance_matrix_cohere<- compute_embedding_distance(cohere_embeddings[1:nrow(cohere_embeddings),2:ncol(cohere_embeddings)],"euclidean")
rownames(distance_matrix_cohere)<-cohere_embeddings$Tumor_Names
colnames(distance_matrix_cohere)<-cohere_embeddings$Tumor_Names

end_time <- Sys.time()
elapsed_time3<- as.numeric(difftime(end_time, start_time, units = "secs"))

print("cohere Distance Matrix Computed")

save.image("cohere.RData")

start_time<-Sys.time()

distance_matrix_cohere<-as.data.frame(distance_matrix_cohere) # EXECUTED


distance_matrix_cohere_v2_all <- distance_matrix_cohere %>% dplyr::select(all_of(WHO_Terms_All$Tumor_Names))
distance_matrix_cohere_v2_5th <- distance_matrix_cohere %>% dplyr::select(all_of(WHO_Terms_5th$Tumor_Names))
distance_matrix_cohere_v2_ncit <- distance_matrix_cohere %>% dplyr::select(all_of(ncit_terms$Tumor_Names))


cohere_match_all<- nearest_match_embeddings(distance_matrix_cohere_v2_all,"cohere") # Executed
cohere_match_5th<- nearest_match_embeddings(distance_matrix_cohere_v2_5th,"cohere") # Executed
cohere_match_ncit<- nearest_match_embeddings(distance_matrix_cohere_v2_ncit,"cohere") # Executed
end_time <- Sys.time()
elapsed_time4<- as.numeric(difftime(end_time, start_time, units = "secs"))

print("cohere Computed")
