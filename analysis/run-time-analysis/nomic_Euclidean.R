method_name <- "nomic+Euclidean Distance"
start_time <- Sys.time()
...
print("nomic Loading")
nomic_embeddings<-read.csv(paste(data_dir,"/Embeddings/nomic-embed-text.csv",sep=""))
nomic_embeddings<-nomic_embeddings[,c(-1)]
end_time <- Sys.time()
elapsed_time<- as.numeric(difftime(end_time, start_time, units = "secs"))

start_time<-Sys.time()
nomic_embeddings<-nomic_embeddings %>% group_by(Tumor_Names) %>% summarise_all("mean")
end_time <- Sys.time()
...
print("nomic Distance Matrix")
distance_matrix_nomic<- compute_embedding_distance(nomic_embeddings[1:nrow(nomic_embeddings),2:ncol(nomic_embeddings)],"euclidean")
rownames(distance_matrix_nomic)<-nomic_embeddings$Tumor_Names
colnames(distance_matrix_nomic)<-nomic_embeddings$Tumor_Names

end_time <- Sys.time()
elapsed_time3<- as.numeric(difftime(end_time, start_time, units = "secs"))

print("nomic Distance Matrix Computed")

save.image("nomic.RData")

start_time<-Sys.time()

distance_matrix_nomic<-as.data.frame(distance_matrix_nomic) # EXECUTED

distance_matrix_nomic_all <- distance_matrix_nomic %>% dplyr::select(all_of(WHO_Terms_All$Tumor_Names))
distance_matrix_nomic_5th <- distance_matrix_nomic %>% dplyr::select(all_of(WHO_Terms_5th$Tumor_Names))
distance_matrix_nomic_ncit <- distance_matrix_nomic %>% dplyr::select(all_of(ncit_terms$Tumor_Names))

print("all match nomic")
nomic_match_all<- nearest_match_embeddings(distance_matrix_nomic_all,"nomic") # Executed
print("5th match nomic")
nomic_match_5th<- nearest_match_embeddings(distance_matrix_nomic_5th,"nomic") # Executed
print("ncit match nomic")
nomic_match_ncit<- nearest_match_embeddings(distance_matrix_nomic_ncit,"nomic") # Executed
end_time <- Sys.time()
elapsed_time4<- as.numeric(difftime(end_time, start_time, units = "secs"))

print("nomic Computed")
