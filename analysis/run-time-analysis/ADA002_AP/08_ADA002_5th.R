#Detect if Affinity cluster members are outliers using LOF and Isolation Forest on ADA2 
...
affinity_cluster_ADA2_df <- read.csv(paste(intermediate_dir,"/affinity_cluster_ADA2_df_5thed.csv",sep="")) # 10 cols
...
save.image(paste(run_time_analysis,"/ADA002_AP/8-ADA002_5th.RData",sep=""))
...
Runtime_8_5th = round(elapsed_time, 2)
Memory_gb_8_5th=round(sum(sizes) / (1024^2), 3)/1000



####
write.csv(affinity_cluster_ADA2_df,paste(intermediate_dir,"/affinity_cluster_ADA2_df_5thed.csv",sep=""))
