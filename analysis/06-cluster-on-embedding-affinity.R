...
lymphoma_leukemia_strings <- c("leukemia", "lymphoma", "leukemias", "lymphomas", "leukaemia", "leukaemias",
                               "leuk")
Lymphoma_Lukemia_Clusters <- affinity_cluster_df %>% dplyr::filter(str_detect(Tumor_Names,paste(lymphoma_leukemia_strings, collapse = "|")))
...
###V3######
Lymphoma_Lukemia_Clusters_V3 <- affinity_cluster_v3_df %>% dplyr::filter(str_detect(Tumor_Names,paste(lymphoma_leukemia_strings, collapse = "|")))

Lymphoma_Lukemia_Clusters_Labels_v3 <- unique(Lymphoma_Lukemia_Clusters_V3$Cluster_ID)

Lymphoma_Lukemia_Clusters_V3 <- affinity_cluster_v3_df %>% dplyr::filter(Cluster_ID %in% Lymphoma_Lukemia_Clusters_Labels_v3 )
###V3######
...
#   
# for (iter_affinity_cluser_v3 in 1: dim(subset_affinity_v3_df)[1]){
#   ind_location_v3 <- which (affinity_cluster_v3_df$Tumor_Names==subset_affinity_v3_df$Tumor_Names[iter_affinity_cluser_v3])
#   affinity_cluster_v3_df$SubsetCluster_IDs[ind_location_v3]<-subset_affinity_v3_df$SubCluster_ID[iter_affinity_cluser_v3]
# }
